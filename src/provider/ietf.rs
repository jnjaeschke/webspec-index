...
// ── String interning ─────────────────────────────────────────────────────────
// Box::leak turns a heap String into a &'static str, but leaks memory.
// The intern cache ensures each unique string is leaked at most once.
...
async fn fetch_doc_meta(
    ...
    if !resp.status().is_success() {
        anyhow::bail!("Datatracker API error for '{}': HTTP {}", name, resp.status());
    }
...
fn extract_doc_name_from_uri(uri: &str) -> Option<&str> {
    uri.trim_end_matches('/').rsplit('/').next().filter(|s| !s.is_empty())
}
...
// ── SpecProvider implementation ───────────────────────────────────────────────

#[async_trait]
impl SpecProvider for IETFProvider {
    fn provider_name(&self) -> &str {
        "ietf"
    }

    /// IETF has no static spec list; all specs are discovered dynamically.
    fn known_specs(&self) -> &[SpecInfo] {
        &[]
    }

    /// Dynamically resolve an IETF RFC or draft name.
    ///
    ...
    async fn find_dynamic_spec(&self, name: &str) -> Result<Option<SpecInfo>> {
        ...
        match parsed.kind {
            IetfDocKind::Rfc => {
                ...
                Ok(Some(make_rfc_spec_info(&terminal)))
            }
            ...
            IetfDocKind::Draft => {
                ...
                if is_expired(&doc) {
                    eprintln!(
                        "Warning: IETF draft '{}' is expired or inactive",
                        base
                    );
                }
                ...
                Ok(Some(SpecInfo {
                    name: intern(canonical_name),
                    base_url: intern(html_url),
                    provider: "ietf",
                    github_repo: "",
                }))
            }
    ...
    /// Return the latest version identifier ("SHA") and date for an IETF document.
    ///
    /// For RFCs the "SHA" is the lowercase RFC name (e.g. "rfc9110"); since RFCs
    /// are immutable, the same value is returned every time, preventing re-indexing.
    /// For drafts the "SHA" is the revision string (e.g. "11"); a new revision
    /// triggers re-indexing.
    async fn fetch_latest_version(&self, spec: &SpecInfo) -> Result<(String, DateTime<Utc>)> {
        let parsed = parse_ietf_name(spec.name);
        let client = make_http_client()?;

        match parsed.kind {
            IetfDocKind::Rfc => {
                let rfc_name = parsed.base.to_lowercase();
                let doc = fetch_doc_meta(&client, &rfc_name)
                    .await?
                    .ok_or_else(|| anyhow::anyhow!("RFC not found: {}", spec.name))?;
                let date = parse_ietf_date(&doc.time)?;
                Ok((rfc_name, date))
            }

            IetfDocKind::Draft => {
                let base = parsed.base;
                let doc = fetch_doc_meta(&client, base)
                    .await?
                    .ok_or_else(|| anyhow::anyhow!("Draft not found: {}", base))?;
                let date = parse_ietf_date(&doc.time)?;
                // Pinned drafts always return the pinned revision as their "SHA"
                let rev = if let Some(pinned) = parsed.pinned_rev {
                    pinned.to_string()
                } else {
                    doc.rev.clone()
    ...
    /// Fetch the HTML for an IETF document.
    ///
    /// `sha` is the revision string for drafts (e.g. "02") or ignored for RFCs.
    /// The URL is always constructed from the spec name + sha, not from base_url,
    /// so that re-indexing picks up the correct new version.
    async fn fetch_html(&self, spec: &SpecInfo, sha: &str) -> Result<String> {
        ...
        let url = match parsed.kind {
            IetfDocKind::Rfc => {
                ...
                format!("https://www.rfc-editor.org/rfc/rfc{}.html", num_str)
            }
            IetfDocKind::Draft => {
                // sha is the revision number; base is the unversioned draft name
                format!(
                    "https://datatracker.ietf.org/doc/html/{}-{}",
                    parsed.base, sha
                )
...
// ── SpecInfo constructors ─────────────────────────────────────────────────────

/// Build a SpecInfo for a published RFC (terminal in its obsoleted_by chain).
/// `rfc_name` is lowercase, e.g. "rfc9187".
pub fn make_rfc_spec_info(rfc_name: &str) -> SpecInfo {
    let lower = rfc_name.to_lowercase();
    let num_str = lower.strip_prefix("rfc").unwrap_or(&lower);
    let num: u32 = num_str.parse().unwrap_or(0);
    SpecInfo {
        name: intern(format!("RFC{}", num)),
        base_url: intern(format!(
            "https://www.rfc-editor.org/rfc/rfc{}.html",
            num
        )),
        provider: "ietf",
        github_repo: "",
...
mod tests {
    ...
    fn provider() -> IETFProvider {
        IETFProvider
    }

    #[test]
    fn test_resolve_datatracker_rfc_html() {
        let r = provider().resolve_url(
    ...
    fn test_resolve_datatracker_draft_html() {
        let r = provider().resolve_url(
            "https://datatracker.ietf.org/doc/html/draft-touch-sne-02#section-1",
        );
        // Version suffix stripped → canonical base name
        assert_eq!(r, Some(("draft-touch-sne".to_string(), "section-1".to_string())));
    }
    ...
    fn test_resolve_rfc_editor_bare() {
        let r = provider()
            .resolve_url("https://www.rfc-editor.org/rfc/rfc9110#section-5");
        assert_eq!(r, Some(("RFC9110".to_string(), "section-5".to_string())));
    }
    ...
    fn test_resolve_rfc_editor_html_ext() {
        let r = provider()
            .resolve_url("https://www.rfc-editor.org/rfc/rfc9110.html#section-5");
        assert_eq!(r, Some(("RFC9110".to_string(), "section-5".to_string())));
    }
    ...
    fn test_resolve_ietf_archive_txt() {
        ...
        assert_eq!(
            r,
            Some(("draft-ietf-dtn-bpsec-default-sc".to_string(), "section-3".to_string()))
        );
    }
    ...
    fn test_resolve_no_fragment() {
        assert_eq!(
            provider().resolve_url("https://www.rfc-editor.org/rfc/rfc9110.html"),
            None
        );
    }
    ...
    fn test_resolve_unknown_host() {
        assert_eq!(
            provider().resolve_url("https://example.com/rfc/rfc9110#foo"),
            None
        );
        assert_eq!(
